...
class Scene(object):
    ...
    def __init__(self):
        ...
        self.groups = []
        self.keyframes = []
        self.frames = 0 # frames of actual gameplay
    ...
    def __mainLoop(self):
        ...
        self.screen.blit(self.background, (0,0))
        if self.bg1: 
            self.bg1offset = [self.bg0offset[0] / 2, self.bg0offset[1] / 2]
            self.screen.blit(self.bg1, self.bg1offset)
        if self.bg0: self.screen.blit(self.bg0, self.bg0offset)
        for group in self.groups:
            #group.clear(self.screen, self.background)
            group.update()
            group.draw(self.screen)
        if self.fg0: self.screen.blit(self.fg0, self.bg0offset)   
...
class SuperSpriteAnim(SuperSprite):
    """ SuperSprite with animation
    """
    ...
    def setState(self, state):
        if (self.currentState != state):
            ...
            self.frame = 0
            # if this state has a nextState
            #if self.

    def addState(self, state, imgFile, imgSize, rows, cols, total, delay, nextState, forwards=True):
        newState = SpriteState(state, imgFile, imgSize, rows, cols, total, delay, nextState, forwards)
        self.states[state] = newState

    def update(self):
        SuperSprite.update(self)
        state = self.states[self.currentState]
        self.pause += 1
        if self.pause >= state.delay:
            self.pause = 0
            # allow for speedups
            if state.delay < 0: self.frame += -state.delay
            else: self.frame += 1
            if self.frame >= len(state.imgList):
                self.frame = 0
                #if another sprite state needs to play...
                if state.nextState != None: 
                    if state.nextState == 'hold': self.frame = len(state.imgList) - 1
                    else: self.setState(state.nextState)
        if state.playForwards:
            frame = self.frame
        else:
            frame = len(state.imgList) - self.frame
        self.image = self.states[self.currentState].imgList[frame]
        self.rect = self.image.get_rect()
        self.rect.center = (self.x, self.y)
        self.image.set_alpha(self.alpha)

    def getCollisionSide(self, other):
        """ return a number indicating the side of a collision
            precondition: a collision must have already been detected
               using collidesWith
            0 = top, 1 = left, 2 = bottom, 3 = right, 4 = inside boundary
            other: SuperSpriteAnim object """
        m = self.rect
        p = other.rect
        if (m.bottom >= p.top) and (m.bottom <= (p.top + self.dy + 1)):
            # go up
            return 0
        elif (m.top <= p.bottom) and (m.top >= (p.bottom + self.dy - 1)):
            # go down
            return 2
        elif (m.right >= p.left) and (m.right <= (p.left + self.dx + 1)):
            # go left
            return 1
        elif (m.left <= p.right) and (m.left >= (p.right + self.dx - 1)):
            # go right
            return 3
        elif (m.right >= (p.left + self.dx)) \
        and (m.left <= (p.right - self.dx)) \
        and (m.bottom >= (p.top + self.dy)) \
        and (m.top <= (p.bottom - self.dy)):
            # inside the boundary
            return 4 
        else:
            # outside bounds
...
class SpriteState():
    """ State object to be owned by a SuperSpriteAnim object
    """

    def __init__(self, state, imgFile, imgSize, rows, cols, total, delay, nextState, forwards):
        self.state = state
        self.imgList = []
        self.imgFile = imgFile
        self.imgSize = imgSize # two-tuple
        self.rows = rows
        self.cols = cols
        self.total = total
        self.nextState = nextState
        self.delay = delay
        self.playForwards = forwards # whether to play forwards or backwards
        self.loadImages()

    def loadImages(self):
        self.imgList = []
        imgMaster = pygame.image.load(self.imgFile)
        imgMaster = imgMaster.convert_alpha()
        for i in range(self.rows):
            for j in range(self.cols):
                if len(self.imgList) == self.total: break
                x = j * self.imgSize[0]
                y = i * self.imgSize[1]
                tmpImg = pygame.Surface(self.imgSize)
                tmpImg.blit(imgMaster, (0, 0), ((x,y), self.imgSize))
